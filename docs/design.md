...
- `color`は6種の列挙値。任意のCSSを保存・適用しない。ブロックにも個別の色を保存し、新規作成時はレーン色を初期値にする。個別色のない既存v1ブロックは読み込み時にレーン色を継承する。
- createdAt / updatedAtはUTCのISO 8601文字列。updatedAtはプロジェクト内容の確定編集で更新。読込・保存・選択・ズームで更新しない。
- 保存対象は名前、テンポ、拍子、総小節数、レーン（名称・色）、ブロック（位置・長さ・ラベル・メモ・個別色）とID・日時。
- 選択状態・ホバー・開いているダイアログ・ズーム・スクロール・未保存フラグはファイルに入れない。再開時は内容を復元し、表示状態は初期値に戻す。
- v1はstrictスキーマ。未知のキーは互換性エラーとして拒否する。ブロック色は旧ファイルに存在しない場合だけレーン色で補完する後方互換フィールドとして扱う。その他の新フィールド追加時はschemaVersionと明示的migrationを検討する。未知の将来版を推測で読み込まない。
