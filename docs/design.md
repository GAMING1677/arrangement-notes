...
作成日: 2026-09-06 / 状態: 設計・基盤準備済み、機能実装前

## 1. 目的と今回の範囲

曲の展開、楽器の出入り、盛り上げ方などを、小節に沿って自由なラベルとメモで記録する。添付DAW画像は上部の小節ルーラー、左側のレーン名、横長ブロックの配置を参考にする。画像内のトラック名・音源・ミキサーは要件ではない。

**今回用意するもの**: React + TypeScript + Vite、shadcn/uiの導入基盤、画面骨格、TypeScriptのデータ契約、サンプルJSON、検査環境、Cloudflare設定、本設計書、実装引き継ぎ書。

**次の実装で作るもの**: レーン追加・改名、BPM等の編集、ブロック配置・移動・伸縮・削除・編集、メモ表示、JSON保存・読み込み、未保存状態の扱い。
...
- 空き場所のダブルクリックでその小節から4小節長の新規編集ダイアログを開く。末尾・次ブロックまでの空きが4未満なら収まる長さにする。空きがなければ作成せず説明する。
- キーボード・タッチ向けにレーンメニューの「ブロック追加」から開始小節と長さを指定可能にする。
- ダイアログにはラベル、メモ、開始小節、長さ（小節）。新規の初期ラベルは「アイデア」。保存時に作成し、キャンセルでは増やさない。
